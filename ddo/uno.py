...
class PointwiseForward(nn.Module):
  ...
  def __call__(self, inputs, height, width):
    ...
    output = jax.image.resize(hidden, (B, height, width, C), method="bilinear")
    return output
...
class UNOResidualBlock(nn.Module):
  ...
  def __call__(self, inputs, times, height, width, is_training):
    ...
    if inputs.shape[-1] != self.n_out_channels and not self.down_or_up_sample:
      residual = nn.Conv(
        self.n_out_channels,
        kernel_size=(1, 1),
        strides=(1, 1),
  ...
  def __call__(self, inputs, times, is_training, **kwargs):
    ...
    for channel_mult, n_modes in zip(
      ...
      for _ in range(self.n_blocks):
        block = UNOResidualBlock(n_outchannels, n_modes, self.dropout_rate)
        hidden = block(
